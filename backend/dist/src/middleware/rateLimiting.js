...
class SecurityStore {
    ...
    checkRateLimit(key, limit) {
        ...
        if ((record === null || record === void 0 ? void 0 : record.blocked) && record.blockUntil && now < record.blockUntil) {
            return {
                allowed: false,
                remaining: 0,
    ...
    recordFailedAttempt(ip) {
        ...
        console.log(`⚠️ FAILED ATTEMPT: ${ip} (${(record === null || record === void 0 ? void 0 : record.count) || 1} total)`);
    }
    ...
    getSecurityMetrics() {
        ...
        return Object.assign(Object.assign({}, this.securityMetrics), { activeUsers: this.securityMetrics.activeUsers.size, recentThreats: recentThreats.length, blockRate: this.securityMetrics.totalRequests > 0 ?
                (this.securityMetrics.blockedRequests / this.securityMetrics.totalRequests * 100).toFixed(2) + '%' : '0%', threatDetectionRate: this.securityMetrics.totalRequests > 0 ?
                (this.securityMetrics.threatsDetected / this.securityMetrics.totalRequests * 100).toFixed(2) + '%' : '0%', uptime: process.uptime(), memoryUsage: process.memoryUsage() });
    }
    // Update metrics
