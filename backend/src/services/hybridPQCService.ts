...
export interface PQCKeyPair {
  publicKey: Buffer;
  secretKey: Buffer;
  algorithm: 'ML-KEM-768' | 'ML-DSA-65' | 'Hybrid-DSA-ECDSA';
  created: Date;
  metadata?: {
...
export interface PQCSignature {
  signature: Buffer;
  algorithm: 'ML-DSA-65' | 'Hybrid-DSA-ECDSA';
  created: Date;
  metadata?: {
    message_hash: string;
    signature_time_ms: number;
  };
}
...
export class ProductionPQCService {
  private static instance: ProductionPQCService;
  ...
  static getInstance(): ProductionPQCService {
   ...
   */
  async encapsulate(publicKey: Buffer): Promise<PQCEncapsulation> {
    const startTime = performance.now();
   ...
   */
  async generateSignatureKeyPair(): Promise<PQCKeyPair> {
    const startTime = performance.now();
   ...
   */
  async generateHybridSignatureKeyPair(): Promise<PQCKeyPair> {
    const startTime = performance.now();
   ...
   */
  async signHybrid(message: Buffer, secretKey: Buffer): Promise<PQCSignature> {
    const startTime = performance.now();
   ...
   */
  async verifyHybrid(message: Buffer, signature: Buffer, publicKey: Buffer): Promise<boolean> {
    try {
...
export const productionPQC = ProductionPQCService.getInstance();

// Legacy compatibility exports (for easy migration)
export const quantumCrypto = productionPQC;
export const pqcService = productionPQC;
