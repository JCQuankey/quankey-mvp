...
exports.generateQuantumPassword = exports.QuantumEntropyService = void 0;
...
class QuantumEntropyService {
     ...
     */
    static async generateQuantumPassword(length = 16, includeSymbols = true) {
        ...
        try {
             ...
             */
            const quantumResponse = await QuantumEntropyService.acquireQuantumEntropy(length, operationId);
            /**
             ...
             */
            password = await QuantumEntropyService.enforceQuantumComplexity(password, includeSymbols, operationId);
            /**
     ...
     */
    static async acquireQuantumEntropy(count, operationId) {
        ...
        for (const source of QuantumEntropyService.QUANTUM_SOURCES) {
            try {
                ...
                if (source.name === 'ANU_QUANTUM_PRIMARY') {
                    const response = await axios_1.default.get(`${source.url}?length=${Math.min(count, source.maxBytes)}&type=uint8`, {
                        timeout: 5000
                    });
                    if (response.data && response.data.success && response.data.data) {
                         ...
                         */
                        const quantumQuality = await QuantumEntropyService.validateQuantumQuality(response.data.data, source.name);
                ...
                if (source.name === 'IBM_QUANTUM_NETWORK') {
                    ...
                    if (response.data && response.data.quantum_data) {
                        const quantumQuality = await QuantumEntropyService.validateQuantumQuality(response.data.quantum_data, source.name);
                ...
                if (source.name === 'CLOUDFLARE_DRAND') {
                    ...
                    if (response.data && response.data.randomness) {
                        ...
                        if (quantumData.length > 0) {
                            const quantumQuality = await QuantumEntropyService.validateQuantumQuality(quantumData, source.name);
     ...
     */
    static async validateQuantumQuality(quantumData, sourceName) {
        /**
     ...
     */
    static async enforceQuantumComplexity(password, includeSymbols, operationId) {
        ...
        const quantumBytes = await QuantumEntropyService.acquireQuantumEntropy(4, operationId);
        const quantumNums = quantumBytes.quantumData || [127, 127, 127, 127];
        if (!hasLower)
            enhanced[quantumNums[0] % enhanced.length] = 'a';
        if (!hasUpper)
            enhanced[quantumNums[1] % enhanced.length] = 'A';
        if (!hasNumber)
            enhanced[quantumNums[2] % enhanced.length] = '1';
        if (!hasSymbol && includeSymbols)
            enhanced[quantumNums[3] % enhanced.length] = '!';
     ...
     */
    static getQuantumPasswordStrength(password, quantumSource = 'UNKNOWN') {
        /**
...
exports.QuantumEntropyService = QuantumEntropyService;
/**
 ...
 */
QuantumEntropyService.QUANTUM_SOURCES = [
    {
 ...
 */
QuantumEntropyService.QUANTUM_CHARSET_LOWER = 'abcdefghijklmnopqrstuvwxyz';
QuantumEntropyService.QUANTUM_CHARSET_UPPER = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
QuantumEntropyService.QUANTUM_CHARSET_NUMBERS = '0123456789';
QuantumEntropyService.QUANTUM_CHARSET_SYMBOLS = '!@#$%^&*()_+-=[]{}|;:,.<>?';
/**
 * ===============================================================================
...
exports.generateQuantumPassword = QuantumEntropyService.generateQuantumPassword;
