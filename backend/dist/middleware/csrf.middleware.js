...
exports.csrfProtection = exports.CSRFProtectionMiddleware = void 0;
...
class CSRFProtectionMiddleware {
     ...
     */
    static validateToken(headerToken, cookieToken) {
        if (!headerToken || !cookieToken)
            return false;
        if (headerToken.length !== this.CSRF_TOKEN_LENGTH * 2)
            return false;
        if (cookieToken.length !== this.CSRF_TOKEN_LENGTH * 2)
            return false;
        // Constant-time comparison to prevent timing attacks
        return crypto_1.default.timingSafeEqual(Buffer.from(headerToken, 'hex'), Buffer.from(cookieToken, 'hex'));
    }
    /**
     * 🌐 VALIDATE ORIGIN
     */
    static validateOrigin(origin, referer) {
...
CSRFProtectionMiddleware.auditLogger = new auditLogger_service_1.AuditLogger();
CSRFProtectionMiddleware.CSRF_TOKEN_LENGTH = 32;
CSRFProtectionMiddleware.ALLOWED_ORIGINS = [
 ...
 */
exports.csrfProtection = CSRFProtectionMiddleware;
