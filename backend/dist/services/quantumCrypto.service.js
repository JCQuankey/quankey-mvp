...
exports.quantumCrypto = exports.QuantumCryptographyService = void 0;
...
class QuantumCryptographyService {
     ...
     */
    async initializeQuantumKeys() {
        try {
            // Generate ML-KEM-768 key pair for encryption
            const kemSeed = (0, utils_js_1.randomBytes)(64); // ML-KEM-768 requires 64 bytes
            this.kemKeyPair = ml_kem_js_1.ml_kem768.keygen(kemSeed);
            // Generate ML-DSA-65 key pair for signatures
            const dsaSeed = (0, utils_js_1.randomBytes)(32); // ML-DSA-65 requires 32 bytes
            this.dsaKeyPair = ml_dsa_js_1.ml_dsa65.keygen(dsaSeed);
            this.auditLogger.logSecurityEvent({
     ...
     */
    async quantumEncrypt(plaintext, recipientPublicKey) {
        try {
            ...
            const encapsulation = ml_kem_js_1.ml_kem768.encapsulate(publicKey);
            ...
            const plaintextBytes = (0, utils_js_1.utf8ToBytes)(plaintext);
            const ciphertext = new Uint8Array(plaintextBytes.length);
            for (let i = 0; i < plaintextBytes.length; i++) {
                ciphertext[i] = plaintextBytes[i] ^ sharedSecret[i % sharedSecret.length];
            }
            // Sign the ciphertext with ML-DSA-65
            const signature = ml_dsa_js_1.ml_dsa65.sign(this.dsaKeyPair.secretKey, ciphertext);
            const result = {
    ...
    async quantumDecrypt(data) {
        try {
            ...
            const isValidSignature = ml_dsa_js_1.ml_dsa65.verify(this.dsaKeyPair.publicKey, ciphertext, signature);
            if (!isValidSignature) {
                throw new Error('Invalid quantum signature - data may be tampered');
            }
            // ML-KEM-768 decapsulation - recover shared secret
            const sharedSecret = ml_kem_js_1.ml_kem768.decapsulate(encapsulatedKey, this.kemKeyPair.secretKey);
     ...
     */
    verifyQuantumSignature(message, signature, publicKey) {
        try {
            const messageBytes = (0, utils_js_1.utf8ToBytes)(message);
            const signatureBytes = new Uint8Array(Buffer.from(signature, 'base64'));
            const publicKeyBytes = new Uint8Array(Buffer.from(publicKey, 'base64'));
            return ml_dsa_js_1.ml_dsa65.verify(publicKeyBytes, messageBytes, signatureBytes);
        }
        catch (error) {
            this.auditLogger.logSecurityEvent({
...
exports.QuantumCryptographyService = QuantumCryptographyService;
/**
 * 🔒 EXPORT SINGLETON INSTANCE
 */
exports.quantumCrypto = new QuantumCryptographyService();
