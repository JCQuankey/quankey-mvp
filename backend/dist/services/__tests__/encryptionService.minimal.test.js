...
(0, globals_1.describe)('🔐 EncryptionService - MINIMAL COVERAGE TEST', () => {
    ...
    (0, globals_1.test)('basic encrypt/decrypt cycle works', async () => {
        ...
        // Encrypt the data
        const encrypted = await encryptionService_1.EncryptionService.encrypt(testData, userCredential);
        ...
        // Decrypt and verify
        const decrypted = await encryptionService_1.EncryptionService.decrypt(encrypted, userCredential);
        (0, globals_1.expect)(decrypted).toBe(testData);
    });
    (0, globals_1.test)('wrong credentials fail decryption', async () => {
        ...
        await (0, globals_1.expect)(async () => {
            await encryptionService_1.EncryptionService.decrypt(encrypted, wrongCredential);
        }).rejects.toThrow();
    });
});
