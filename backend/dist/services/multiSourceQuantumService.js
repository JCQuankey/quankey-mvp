...
exports.MultiSourceQuantumService = void 0;
const axios_1 = __importDefault(require("axios"));
const crypto_1 = __importDefault(require("crypto"));
class MultiSourceQuantumService {
    /**
     * PATENT-CRITICAL: ANU QRNG - Vacuum Fluctuation Quantum Generator
     */
    static async getANUQuantumRandom(count) {
        const startTime = Date.now();
        try {
            const response = await axios_1.default.get(`https://qrng.anu.edu.au/API/jsonI.php?length=${count}&type=uint8`, { timeout: 5000 });
     ...
     */
    static async getIBMQuantumRandom(count) {
        const startTime = Date.now();
        try {
            ...
            const response = await axios_1.default.post('https://api.quantum-computing.ibm.com/api/v1/jobs', {
     ...
     */
    static async getCloudflareDrand(count) {
        const startTime = Date.now();
        try {
            // Cloudflare drand provides verifiable randomness
            const response = await axios_1.default.get('https://drand.cloudflare.com/public/latest', { timeout: 3000 });
            const latency = Date.now() - startTime;
            if (response.data && response.data.randomness) {
                ...
                const expanded = crypto_1.default.createHmac('sha256', beacon)
     ...
     */
    static async getIntelRDRAND(count) {
        const startTime = Date.now();
        try {
            // Intel RDRAND via Node.js crypto (which uses hardware when available)
            const randomBytes = crypto_1.default.randomBytes(count);
            const latency = Date.now() - startTime;
     ...
     */
    static async generateQuantumRandom(count) {
        ...
        const fallbackBytes = crypto_1.default.randomBytes(count);
        return {
...
exports.MultiSourceQuantumService = MultiSourceQuantumService;
MultiSourceQuantumService.sources = new Map([
