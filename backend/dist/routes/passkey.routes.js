 ...
 */
router.post('/register/begin', inputValidation_middleware_1.inputValidation.validatePasskeyRegister(), async (req, res) => {
    try {
...
router.post('/auth/complete', async (req, res) => {
    try {
        ...
        const authToken = (0, crypto_1.randomBytes)(32).toString('hex');
        await prisma.session.create({
            data: {
                token: authToken,
