 ...
 */
router.post('/test-encrypt', inputValidation_middleware_1.inputValidation.validateQuantumTest(), async (req, res) => {
    try {
 ...
 */
router.get('/validate-real', async (req, res) => {
    try {
        const quantumStatus = quantumCrypto_service_1.quantumCrypto.getQuantumStatus();
        const encryptionStatus = encryption_service_1.encryption.getQuantumStatus();
        const validation = {
        ...
        const allValidationsPassed = validation.goldenRuleCompliance.noSimulations &&
