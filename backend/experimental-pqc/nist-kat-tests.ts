...
import { pqcIntegrationService } from '../services/pqcIntegrationService';
import * as crypto from 'crypto';
import * as fs from 'fs/promises';
import * as path from 'path';

export interface KATTestVector {
  algorithm: string;
  operation: string;
  testNumber: number;
  seed?: string;
...
export interface KATTestResults {
  algorithm: string;
  totalTests: number;
  passedTests: number;
  failedTests: number;
 ...
 */
export class NISTKATTester {
  private testVectors: KATTestVector[] = [];
  private results: Map<string, KATTestResults> = new Map();
   ...
   */
  public async loadOfficialTestVectors(testDataDirectory: string): Promise<void> {
    try {
   ...
   */
  private async parseKATFile(filePath: string, algorithm: string): Promise<KATTestVector[]> {
    const content = await fs.readFile(filePath, 'utf8');
    const vectors: KATTestVector[] = [];
    const lines = content.split('\n');
    
    let currentVector: Partial<KATTestVector> = {};
    let testNumber = 1;
   ...
   */
  public async runAllTests(): Promise<Map<string, KATTestResults>> {
   ...
   */
  public async runTestsForAlgorithm(algorithm: string): Promise<KATTestResults> {
    const startTime = Date.now();
    const vectors = this.testVectors.filter(v => v.algorithm === algorithm);
    
    const result: KATTestResults = {
      algorithm,
   ...
   */
  private async runSingleTest(vector: KATTestVector): Promise<boolean> {
    try {
   ...
   */
  private async testMLKEM768(vector: KATTestVector): Promise<boolean> {
    switch (vector.operation) {
      case 'keypair':
        // Test key pair generation
        const keyPair = await pqcIntegrationService.generateKEMKeyPair();
        return keyPair.publicKey.length === 1184 && keyPair.secretKey.length === 2400;

      case 'encapsulation':
        if (!vector.publicKey) return false;
        const publicKey = Buffer.from(vector.publicKey, 'hex');
        const encapsulation = await pqcIntegrationService.encapsulateSecret(publicKey);
        return encapsulation.ciphertext.length === 1088 && encapsulation.sharedSecret.length === 32;

      case 'decapsulation':
        if (!vector.secretKey || !vector.ciphertext) return false;
        const secretKey = Buffer.from(vector.secretKey, 'hex');
        const ciphertext = Buffer.from(vector.ciphertext, 'hex');
        const sharedSecret = await pqcIntegrationService.decapsulateSecret(ciphertext, secretKey);
   ...
   */
  private async testMLDSA65(vector: KATTestVector): Promise<boolean> {
    switch (vector.operation) {
      case 'keypair':
        // Test key pair generation
        const keyPair = await pqcIntegrationService.generateSignatureKeyPair();
        return keyPair.publicKey.length === 1952 && keyPair.secretKey.length === 4000;

      case 'signing':
        if (!vector.message || !vector.secretKey) return false;
        const message = Buffer.from(vector.message, 'utf8');
        const secretKey = Buffer.from(vector.secretKey, 'hex');
        const signature = await pqcIntegrationService.signMessage(message, secretKey);
        return signature.length > 0 && signature.length <= 3293;

      case 'verification':
        if (!vector.message || !vector.signature || !vector.publicKey) return false;
        ...
        const isValid = await pqcIntegrationService.verifySignature(msgBuffer, sigBuffer, pubKeyBuffer);
  ...
  public async runSelfTest(): Promise<boolean> {
    ...
    try {
      ...
      const serviceTest = await pqcIntegrationService.runSelfTest();
      ...
      const kemKeyPair = await pqcIntegrationService.generateKEMKeyPair();
      const encapsulation = await pqcIntegrationService.encapsulateSecret(kemKeyPair.publicKey);
      const decapsulatedSecret = await pqcIntegrationService.decapsulateSecret(
      ...
      const sigKeyPair = await pqcIntegrationService.generateSignatureKeyPair();
      const testMessage = Buffer.from('NIST KAT self-test message', 'utf8');
      const signature = await pqcIntegrationService.signMessage(testMessage, sigKeyPair.secretKey);
      const isValid = await pqcIntegrationService.verifySignature(testMessage, signature, sigKeyPair.publicKey);
   ...
   */
  public async exportResults(outputPath: string): Promise<void> {
    const reportData = {
      ...
      performanceMetrics: pqcIntegrationService.getPerformanceMetrics()
    };
...
export const nistKATTester = new NISTKATTester();