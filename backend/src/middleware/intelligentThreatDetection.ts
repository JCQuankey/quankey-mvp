...
class IntelligentSecurityStore {
  private static instance: IntelligentSecurityStore;
  private requestCounts: Map<string, { count: number; firstRequest: number; blocked: boolean; blockUntil?: number }> = new Map();
  private legitimateSessions: Map<string, { userAgent: string; patterns: string[]; score: number; established: number }> = new Map();
  private threatIntelligence: Map<string, { count: number; severity: number; lastSeen: number }> = new Map();
  ...
  static getInstance(): IntelligentSecurityStore {
  ...
  checkSmartRateLimit(key: string, limit: any, requestContext: { isLegitimate: boolean; endpoint: string }): any {
    const now = Date.now();
  ...
  analyzeIntelligentThreats(ip: string, userAgent: string, path: string, method: string, body: any): {
    isLegitimate: boolean;
    riskScore: number;
    threatType: string;
    action: 'allow' | 'warn' | 'investigate' | 'block';
  } {
    let riskScore = 0;
    let threatType = '';
    ...
    const sessionKey = `session:${ip}:${createHash('md5').update(userAgent).digest('hex').substring(0, 8)}`;
    ...
    let action: 'allow' | 'warn' | 'investigate' | 'block';
  ...
  private isLegitimateOperation(path: string, userAgent: string, method: string): boolean {
  ...
  updateThreatIntelligence(ip: string, severity: number): void {
    const key = `threat:${ip}`;
  ...
  // Update metrics
  updateMetrics(analysis: any): void {
...
export function createIntelligentRateLimiter(type: keyof typeof INTELLIGENT_SECURITY_CONFIG.smartRateLimits) {
...
export function intelligentSecurityMiddleware(req: Request, res: Response, next: NextFunction) {
  const store = IntelligentSecurityStore.getInstance();
  const ip = req.ip || req.connection.remoteAddress || 'unknown';
  const userAgent = req.get('User-Agent') || '';
...
export function intelligentSecurityMetrics(req: Request, res: Response) {
  const store = IntelligentSecurityStore.getInstance();
...
export { INTELLIGENT_SECURITY_CONFIG, IntelligentSecurityStore };
